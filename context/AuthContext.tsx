...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const hasSubscriptionExpired = async (subscriptionId: string) => {
    ...
    try {
      ...
      if (subscriptionData?.subscription?.status == 'expired' || subscriptionData?.subscription?.status == 'cancelled') {
        return true;
      }
      return false;
  ...
  const fetchPlanById = async (id: string) => {
    try {
      ...
      if (subscriptionId) {
        const expired = await hasSubscriptionExpired(data?.plan?.subscriptionid);
        ...
        if (expired) {
          if (new Date(data?.plan?.monthenddate) < currentDate) {
            ...
            const planres = await fetch(`/api/plan-details/${data?.plan?.id}`, {
              method: 'POST',
              body: JSON.stringify({ name: 'Free', monthly_limit: 5, subscriptionID: '', MonthEndDate: nextYearDate.toISOString() }),
            });
  ...
  return (
    <AuthContext.Provider value={{ currentUser, setCurrentUser, fetchUser, creditsData, setCreditsData, hasExpired, setDisabledCards,disabledCards }}>
      {children}
